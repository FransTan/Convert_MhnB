...
class SafeFieldParser(FieldParser):
    """FieldParser that tolerates malformed/blank date and time fields.

    Some real-world DBF exports store "empty" date fields as raw
    null bytes (\\x00...) instead of spaces or ASCII zeros. dbfread's
    default parser only recognizes spaces/'0' as blank and raises
    ValueError on anything else it can't parse as a real date. Here,
    any date/time field that fails to parse is simply treated as a
    missing value (None) rather than aborting the whole conversion.
    """
    ...
    def parseT(self, field, data):
        try:
            return super().parseT(field, data)
        except Exception:
            return None
