...
class SafeFieldParser(FieldParser):
    """FieldParser that tolerates malformed/blank field values.

    Some real-world DBF exports store "empty" values as raw null bytes
    (\\x00...) instead of the spaces or ASCII zeros dbfread expects for
    blanks. This affects date fields, but can also affect numeric and
    other field types in the same file. Rather than patching each field
    type individually, override the top-level parse dispatch: if a
    field's value can't be parsed for any reason, treat it as missing
    (None) instead of aborting the entire conversion.
    """
...
if uploaded_file is not None:
    ...
    try:
        ...
        # embedded inside otherwise normal text fields. Excel's XML format
        # cannot store these, so openpyxl raises IllegalCharacterError on
        # write. Strip them from every column by checking each individual
        # value rather than relying on the column's pandas dtype: depending
        # on the pandas version/backend, a pure-string column may be typed
        # as "object", "string", or "str" rather than always "object", so a
        # dtype-based filter can silently skip columns that need cleaning.
        def _strip_illegal_chars(value):
            if isinstance(value, str):
                return ILLEGAL_CHARACTERS_RE.sub("", value)
            return value

        for col in df.columns:
            df[col] = df[col].apply(_strip_illegal_chars)
